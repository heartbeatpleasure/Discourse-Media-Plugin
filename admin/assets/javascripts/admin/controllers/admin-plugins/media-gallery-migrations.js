...
export default class AdminPluginsMediaGalleryMigrationsController extends Controller {
  ...
  get cleanupDisabled() {
    return !this.hasSelectedItem || this.isCleaning || this.isCopying || this.isSwitching || this.isRollingBack || this.isFinalizing;
  }
  ...
  get rollbackDisabled() {
    return !this.hasSelectedItem || this.isRollingBack || this.isCopying || this.isSwitching || this.isCleaning || this.isFinalizing;
  }

  get finalizeDisabled() {
    return !this.hasSelectedItem || this.isFinalizing || this.isRollingBack || this.isCopying || this.isSwitching;
  }

  get bulkMigrateDisabled() {
    return this.isBulkMigrating || this.isSearching || this.sortedResults.length === 0;
  }
  ...
  async refreshSelected() {
    ...
    try {
      ...
      if (targetProfileKey && sourceProfileKey && targetProfileKey === sourceProfileKey) {
        this.selectedPlanError = "Source and target use the same profile. Change the target profile to preview a migration.";
        this.selectedPlan = null;
      } else {
        try {
  ...
  async bulkMigrate() {
    ...
    this.bulkActionMessage = "";
    try {
      const payload = Object.fromEntries(this._currentSearchParams().entries());
      payload.target_profile = "target";
      payload.force = this.forceAction;
      payload.auto_switch = this.autoSwitch;
      payload.auto_cleanup = this.autoCleanup;
      ...
      this.bulkActionMessage = `${result?.queued_count || 0} item(s) queued, ${result?.skipped_count || 0} skipped.`;
      await this.search();
      if (this.hasSelectedItem) {
        await this.refreshSelected();
      }
